...
async def settings(nav: Navigator):
    ...
    await nav.send_message(Texts.settings_text(nav.user.time_to_ask,
        nav.user.rating_publicity), keyboard=Keyboards.settings_menu)
...
@dp.message_handler(match_text(Texts.yes_button), state=States.settings_waiting_for_rating)
@with_navigator
...
@dp.message_handler(match_text(Texts.no_button), state=States.settings_waiting_for_rating)
@with_navigator
async def set_yes_rating(nav: Navigator):
